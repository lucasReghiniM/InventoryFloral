      ...
      <CardContent className="pt-6">
        ...
        <div className="overflow-x-auto">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="px-4 py-3 text-left text-xs font-medium text-neutral-800 uppercase tracking-wider">Invoice #</TableHead>
                <TableHead className="px-4 py-3 text-left text-xs font-medium text-neutral-800 uppercase tracking-wider">Date</TableHead>
                <TableHead className="px-4 py-3 text-left text-xs font-medium text-neutral-800 uppercase tracking-wider">Supplier</TableHead>
                <TableHead className="px-4 py-3 text-left text-xs font-medium text-neutral-800 uppercase tracking-wider">Total</TableHead>
            ...
            <TableBody>
              {isLoading ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-4">Loading purchases...</TableCell>
                </TableRow>
              ) : purchases.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-4">No purchases found</TableCell>
                </TableRow>
              ) : (
                purchases.map((purchase) => (
                  <TableRow key={purchase.id}>
                    <TableCell className="px-4 py-3 whitespace-nowrap text-sm text-neutral-800">{purchase.invoiceNumber}</TableCell>
                    <TableCell className="px-4 py-3 whitespace-nowrap text-sm text-neutral-800">
                      {format(new Date(purchase.orderDate), 'yyyy-MM-dd')}
                    </TableCell>
                    <TableCell className="px-4 py-3 whitespace-nowrap text-sm text-neutral-800">{purchase.supplier}</TableCell>
